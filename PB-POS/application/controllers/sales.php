...
class Sales extends Secure_area {
    ...
    function edit($sale_id) {
        ...
        $data['is_employee'] = $this->Employee->has_auth('2', $person_info->person_id);
        $data['is_manager'] = $this->Employee->has_auth('1', $person_info->person_id);
        $data['is_owner'] = $this->Employee->has_auth('0', $person_info->person_id);
    ...
    function _reload($data = array()) {
        ...
        $data['payments'] = $this->sale_lib->get_payments();
        $data['person_id'] = $person_info->person_id;
        $data['shiftstarted'] = $this->shift_exists($person_info->person_id);
    ...
    function viewrecent() {
        ...
        $data = array();
        $data['recent_sales'] = $model->getData(array('start_date' => date('Y-m-d', strtotime('now')), 'end_date' => date('Y-m-d', strtotime('now')), 'sale_type' => 'all'));
        $this->load->view('sales/recent', $data);
    }

    function startshift($person_id) {
        $data = array(
            'person_id' => $person_id
        );

        $this->load->view('shifts/shiftopen', $data);
    }
    ...
    function shift_exists($person_id) {
//        $this->load->model('shift');
//        $model = $this->shift;
//        return $model->shift_open($person_id);
        $query = $this->db->query("SELECT * FROM ospos_shifts WHERE employee_id=$person_id AND end_time='0000-00-00 00:00:00'");
        //$query = $this->db->get_where('shifts', array('employee_id' => $person_id, 'end_time' => '0000-00-00 00:00:00'));
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
