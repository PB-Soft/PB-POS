...
class Employee extends Person {
    ...
    /*
      Determins the authorization level of employee.
     */

    function has_auth($authlevel, $person_id) {
        //if no module_id is null, allow access
        if ($authlevel == null) {
            return true;
        }

        $query = $this->db->get_where('employees', array('person_id' => $person_id, 'authlevel' => $authlevel), 1);
        return $query->num_rows() == 1;
    ...
    function get_auth($person_id) {
        $query = $this->db->get_where('employees', array('person_id' => $person_id), 1)->result_array();
        return $query[0]['authlevel'];
