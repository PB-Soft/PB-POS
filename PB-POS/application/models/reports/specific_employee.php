	...
	public function getData(array $inputs)
	{
		...
		foreach($data['summary'] as $key=>$value)
		{
			...
			$data['details'][$key] = $this->db->get()->result_array();
                ...
                $this->db->from('shifts');
                $this->db->where("employee_id = \"{$inputs['employee_id']}\" AND start_time BETWEEN \"{$inputs['start_date']} 0000-00-00 00:00:00\" and \"{$inputs['end_date']} 0000-00-00 00:00:00\"");
                $data['shifts'] = $this->db->get()->result_array();             
		return $data;
	}
